...
public class LauncherConfig {
	...
	public LauncherConfig(GameEngine engine) {
		...
		this.launcherConfig=new File(engine.getGameFolder().getBinDir(), "launcher_config.json");boolean f=this.launcherConfig.exists();
		...
		if (!f==false) {
			...
			//configDetails.put(EnumConfig.CONNECT.getOption(), EnumConfig.CONNECT.getDefault());
	...
	public void loadConfiguration() {
		String json=null;
		try {
			/*URL u1=new URL(this.launcherConfig.toURI().toURL().toString());
			String url=u1.getFile().substring(1, u1.getFile().length());
			System.out.println(url);
			//System.out.println(JsonUtil.loadJSON(u1));*/
